...
``--localdirpath``
        (optional) path where to create a local sync folder when creating an account via command-line. If skipped, then default local sync folder path (/home/Nextcloud<n>) will be generated by desktop client.
